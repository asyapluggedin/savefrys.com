...
(function(){
    function init(){
        ...
        function print(){
            console.log('print');
            printJS('http://marianopascual.me/PrintArtwork.pdf', 'pdf');
            location.hash = '';
        }
...
(function(){
    var topNavCon = document.querySelector('.top-nav'),
        ...
        slideshowButtons = document.querySelectorAll('.slideshow__pjt-button'),
    ...
    if(window.innerWidth < 1088){
        loadImages(document.querySelector('#latestwork'));
    }
})();
...
(function(){
    ...
    document.querySelector('.latestwork-btn').addEventListener('click', detail.open);

    for(i = 0; i < folderLinks.length; i++){
        folderLinks[i].addEventListener('click', detail.open);
    }
    for(i = 0; i < slideshowButtons.length; i++){
        slideshowButtons[i].addEventListener('click', detail.open);
    }
    for(i = 0; i < closeButtons.length; i++){
...
},{}]},{},[8,9,4,5,15,10,11,3,12,7,13,14,1,6,2]);